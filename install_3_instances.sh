...
cfg_node() {
  	set timeout 300
  	spawn docker run -it -v $(cd ~/.$1; pwd):/root/.streamr streamr/broker-node:testnet bin/config-wizard
  	...
  	docker run -it --restart=always --name=streamr_node -d -p $2:$2 -p $4:$4 -p $3:$3 -v $(cd ~/.$1; pwd):/root/.streamr streamr/broker-node:testnet
